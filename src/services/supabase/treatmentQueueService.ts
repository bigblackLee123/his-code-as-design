...
export const treatmentQueueService = {
  ...
  async getTreatmentQueueByRegion(region: string): Promise<QueueItem[]> {
    ...
    // 2. 查该区域仍有 pending/in-progress 步骤的 consultation_id
    const { data: steps, error: stepError } = await supabase
      .from("prescription_steps")
      .select("prescription_id, prescriptions!inner(consultation_id)")
      .eq("region", region)
      .in("status", ["pending", "in-progress"]);

    throwIfError(stepError, { table: "prescription_steps", operation: "select" });

    const activeConsultationIds = new Set(
      (steps ?? []).map((s: any) => s.prescriptions.consultation_id)
    );

    // 4. 只返回在该区域还有未完成步骤的队列项
    return items.filter((i) => activeConsultationIds.has(i.consultationId));
  },
